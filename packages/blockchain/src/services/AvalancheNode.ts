...
export class AvalancheNode {
  ...
  constructor(config: AvalancheConfig) {
    ...
    this.logger = winston.createLogger({
      ...
      transports: [
        new winston.transports.File({ filename: 'avalanche.log' })
      ]
    });
  ...
  private createProvider(config: AvalancheConfig): ethers.Provider {
    if (config.wsUrl) {
  ...
  private initializeSigner(credentials: AvalancheConfig['credentials']) {
  ...
  async deployContract(abi: any[], bytecode: string, ...args: any[]): Promise<ethers.Contract> {
    ...
    await this.redis.hset(
      `avalanche:contracts:${this.config.chainId}`,
      address,
      JSON.stringify({
        abi,
        address,
