...
const BlockchainNode = ({ data, isConnectable }: NodeProps<BlockchainNodeData>) => {
  ...
  // Map blockchain status to NetworkStatusIndicator status
  ...
  const getNetworkIcon = (networkType: string) => {
    switch (networkType) {
      ...
      case 'avalanche':
        return (
          <svg className="w-5 h-5 text-red-400" viewBox="0 0 24 24">
            <path fill="currentColor" d="M12 2L2 7v10l10 5 10-5V7L12 2zm0 2.8L19.2 8 12 11.2 4.8 8 12 4.8zM4 16.2V9.8l7 3.5v6.4l-7-3.5zm16 0l-7 3.5v-6.4l7-3.5v6.4z"/>
          </svg>
        );
      case 'polkadot':
  ...
  return (
    <AnimatedCard
      ...
      borderColor={data.status === 'error' ? 'rgba(220, 38, 38, 0.5)' : 'rgba(37, 99, 235, 0.5)'}
      glowColor={data.status === 'error' ? 'rgba(220, 38, 38, 0.3)' : 'rgba(37, 99, 235, 0.3)'}
    >
                ...
                <>
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="flex items-center">
                      ...
                      <AnimatedCounter 
                        value={data.metrics.blockNumber} 
                        size="sm" 
                        color="primary"
                      />
                    </span>
                  ...
                  {data.metrics.syncProgress !== undefined && (
                    <div className="mt-2">
                      <span className="text-xs text-blue-300">Sync Progress:</span>
                      <AnimatedProgressBar 
                        value={data.metrics.syncProgress} 
                        maxValue={100}
                        height={6}
                        color="primary"
                        showValue={true}
                  ...
                  {data.metrics.transactionsPerSecond !== undefined && (
                    <div className="flex justify-between text-xs">
                      <span className="text-blue-300">TPS:</span>
                      <AnimatedCounter 
                        value={data.metrics.transactionsPerSecond} 
                        size="sm" 
                        color="primary"
                        decimals={1}
                        suffix=" tx/s"
